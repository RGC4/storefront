...
export default function ProductGallery({ images }: { images: string[] }) {
  ...
  // DESKTOP — original thumbnail sidebar layout
  ...
  return (
    <div style={{ display: "flex", gap: "12px", width: "100%", height: "638px" }}>
      <div style={{ width: "143px", flexShrink: 0, display: "flex", flexDirection: "column", gap: "8px" }}>
        {canScrollUp && (
          <button onClick={() => setThumbStart(s => Math.max(0, s - 1))}
            style={{ height: "32px", border: "1px solid #ddd", background: "#f8f8f8", cursor: "pointer", fontSize: "14px", color: "#555", borderRadius: "4px" }}>↑</button>
        )}
        {visibleImages.map((url, i) => {
          ...
          return (
            <div key={globalIndex} onClick={() => setCurrent(globalIndex)}
              style={{
                flex: 1, border: isSelected ? "2px solid #111" : "1px solid #ddd",
              ...
              <img src={thumbSrc(url)} alt={`view ${globalIndex + 1}`}
                style={{ width: "100%", height: "100%", objectFit: "contain", objectPosition: "center", padding: "8px", display: "block", pointerEvents: "none" }} />
            </div>
          );
        })}
        {canScrollDown && (
      ...
      <div ref={mainRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}
        style={{
          ...
          overflow: "hidden", cursor: "crosshair", backgroundColor: "white",
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>
        <img ref={imgRef} src={mainSrc(images[current])} alt="product"
          style={{
            maxWidth: "100%", maxHeight: "100%", width: "auto", height: "auto",
            objectFit: "contain", objectPosition: "center", padding: "16px",
