...
import { CartLine as CartModel } from "contexts/CartContext";
...
export default function CartItem({ item }: Props) {
  const { lineId, title, price, thumbnail, slug, qty } = item;

  const { updateCartLine, removeCartLine } = useCart() as any;
    ...
    <Wrapper elevation={0}>
      ...
      <ContentWrapper>
        ...
        <div className="quantity-buttons-wrapper">
          <QuantityButton
            disabled={qty === 1}
            onClick={() => updateCartLine(lineId, Math.max(1, qty - 1))}
          >
          ...
          <QuantityButton
            disabled={qty === 10}
            onClick={() => updateCartLine(lineId, qty + 1)}
          >
            <Add fontSize="small" />
          </QuantityButton>
        </div>
